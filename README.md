**项目地址**
    ...
    本仓库是上游项目在 AGPL-3.0 下的自用分支，定位为**中小学教务管理**，主要差异：
      · 术语统一为 学生 / 教师 / 学校（上游为 学员 / 导师 / 机构）
      · 移除课时费模块（课费项、缴费、退费、催缴提醒、课费报表）——义务教育阶段
        不存在按课时向学生收费的场景；相关接口、报表与运营大屏收入卡片已一并移除
      · 已移除高级功能的授权门禁，其余功能全部开放
      · 已移除向供应商回传学校/联系人/机器码的通道
      · 新增 MCP 服务器，可对接 AstrBot 等第三方框架
      · 镜像构建改为多组件矩阵 + 多架构（amd64 / arm64）

    ⚠️ 若你需要课时费、试读转化等培训业务功能，请直接使用上游版本。
...
**项目功能介绍**
    ...
    本系统是一套面向中小学的教务管理平台，涵盖排课调度、学生与教师管理、考勤、成绩与素质评价的全流程业务。
    上游按授权方式把功能分为「默认授权」与「高级授权」两类；本分支已放开全部功能，
    下文保留该分类只是为了说明功能归属，实际使用时无需任何激活操作。
    注：上游的「费用管理」已整体移除，功能清单中不再列出。

    **默认授权 基础功能**
        📚 科目与排课
            科目管理：科目代码/名称维护、批量导入
            手动排课：按教师/班级/教室/科目灵活排课，冲突自动检测
            排课状态跟踪：待上课、已完课、延期、取消
            学生考勤：出席/请假/缺席签到，缺席自动标记待补课
            日历视图与表格视图双模式展示课程安排
            约束条件管理：硬性约束（教师/学生/班级/教室时间约束）与软性约束
            节假日管理：假期维护与批量导入，排课自动避开节假日
        👨‍🏫 教师与学生
            教师管理：信息维护、科目关联、在职/离职状态
            学生管理：档案维护、班级归属、多维度筛选与搜索
            班级管理：班级信息、学生分配、试读/正式课标记
            教室管理：教室代码/名称/位置/容量维护
            请假管理：教师/学生请假记录、请假后排课自动规避
            教师可见性控制：开启后教师仅可见自己相关的科目、班级与排课
        ⚙️ 系统管理
            用户管理：多角色权限（超级管理员/系统管理员/课程管理员/系统审计员）三权分立
            教师角色授权：成绩管理教师、评价管理教师、运营管理教师
            LDAP统一认证：支持LDAP/AD域账号登录，角色自动映射
            开放注册：限时开放用户自助注册（3天自动关闭）
            登录超时配置：自定义Token过期时间（5分钟~30天）
            密码重置：用户自助申请、管理员审核重置
            邮箱通知：SMTP邮件向课程安排关联的教师/学生发送课程安排提醒和课程变更提醒
            系统日志：操作日志记录、日志归档备份与清除
            站点信息：学校名称/LOGO/官网/二维码等全局参数配置
        📋 排课管理
            手动排课：灵活创建课程安排
            排课查询：按条件筛选查看排课记录
            请假/调课：便捷处理临时变动
    ...
    ✅ 本分支已移除授权门禁，以下功能全部开箱可用，无需激活、无需机器码、无需联网校验。
       ...
       📖 移除了什么、如何还原门禁：docs/PREMIUM.md
        ...
        📊 学生评价管理
            ...
            雷达图可视化：ECharts五维/多维雷达图直观展示学生能力结构
            评价管理教师：独立授权评价管理权限，精细控制访问范围
        💬 微信通知管理
            企业微信群机器人Webhook通知
            课程安排提醒：新建、更新、完课、延期、取消等信息向教师信息群/班级信息群分别推送
            综合管理通知：教务管理相关信息推送
        🤖 智能指令管理
            自然语言指令解析：一句话完成添加科目/教师/学生/排课/请假等操作
            双模式解析：规则解析（内置）与AI大模型解析（须配置大模型API KEY）（Deepseek/千问/OpenAI/自定义）
            指令示例管理：自定义智能指令，同时内置示例库，支持批量AI/正则对比测试
        📺 运营大屏
            KPI看板：收入/转化率/学生数/教师数/课次/出勤率等核心指标
            可视化图表：科目分布、教师工作量、收入相关图表分析（可一键隐藏）、月度趋势等
            试读转化漏斗、教师效能排行
            全屏模式展示，支持导出图片
        🔮 全站快捷按钮
...
**🔌 MCP 服务器（对接 AstrBot 等第三方框架）**
    ...
    本项目内置一个 MCP（Model Context Protocol）服务器，把排课/学生/教师/成绩/统计
